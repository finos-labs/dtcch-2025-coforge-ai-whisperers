...
def process_and_log_corporate_actions():
    """ Fetch, log execution details, and post corporate actions """
    start_time = time.time()

    print("Fetching Corporate Actions data...")
    ...
    log_data = {
        "Scheduler_Name": "Bussiness Wire Fetcher",
    ...
    if extracted_data:
        for record in extracted_data:
            try:
                ...
                print(f"Corporate Action | Status Code: {response.status_code}, Response: {response.json()}")
            except Exception as e:
                print(f"Error posting Corporate Action data: {e}")

# Schedule the job every 5 minutes
schedule.every(5).minutes.do(process_and_log_corporate_actions)
